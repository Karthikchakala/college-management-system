  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-black text-slate-800 tracking-tight">My Academic Catalog</h1>
        <p className="text-sm text-slate-500 font-medium">Verify your enrolled courses, syllabus descriptions, and credit metrics.</p>
      </div>
          ...
          <div key={c.id} className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-between space-y-4 hover:shadow-md transition">
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-slate-400">
              ...
              <span className="text-primary-600 font-bold">
                Instructor: {c.faculty ? `${c.faculty.firstName} ${c.faculty.lastName}` : 'Not Assigned'}
              </span>
            </div>
          </div>
        ))}
