...
export default function Login() {
  ...
  const cognitoDomain = import.meta.env.VITE_COGNITO_DOMAIN || 'https://us-east-1ic9huqjjl.auth.us-east-1.amazoncognito.com';
  const cognitoClientId = import.meta.env.VITE_COGNITO_CLIENT_ID || '3kv2vgpkklqtlpfom2t72dn29n';
  const redirectUri = import.meta.env.VITE_COGNITO_REDIRECT_URI || window.location.origin;

  const handleCognitoLogin = () => {
    const hostedUiUrl = `${cognitoDomain}/login?client_id=${cognitoClientId}&response_type=token&scope=email+openid+profile&redirect_uri=${encodeURIComponent(redirectUri)}`;
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-900 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(14,165,233,0.15),rgba(255,255,255,0))] font-sans antialiased text-slate-200 px-4">
      <div className="w-full max-w-md p-8 bg-slate-950/40 backdrop-blur-md rounded-2xl border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <button
          ...
          className="w-full mb-5 py-3.5 px-4 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-300 font-bold text-sm shadow-md transition duration-150 flex items-center justify-center gap-2.5 hover:scale-[1.01] active:scale-[0.99]"
        >
